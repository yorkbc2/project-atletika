(function ($) {
    ...
    $(function () {
        ...
        // Buy one click
        buyOneClick('.one-click', '[data-field-id="field11"]', 'h1.entry-title');

        var removeButtonContainers = $('.product-remove');

        if ($('.slick-slider').is('div'))
            $('.slick-slider').slick({
                dots: true,
                infinite: true,
                speed: 600,
        ...
        if (removeButtonContainers.length > 0)
        {
            removeButtonContainers.each(function ()
            {
                var row = $(this).parent();
                var button = $(this).find('a.remove');
    ...
    window.incrementBasketCount = function(event) {
        $('.basket-count')
            .each(function() {
                $(this).text(
