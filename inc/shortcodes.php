...
if (!function_exists('wc_categories_with_child'))
{
    function wc_categories_with_child()
    {
        return the_woocommerce_categories_with_child(false);
    }

    add_shortcode( 'bw-categories', 'wc_categories_with_child' );
}

if (!function_exists('reviews_slider_shortcode'))
{
    function reviews_slider_shortcode($atts)
    {
        $output = '<div class="slick-slider">';
        foreach (get_reviews() as $post)
        {
            $output .= '<div class="slide review-slide">
                '. get_the_post_thumbnail($post->ID, 'full') .'
                <h5>'. $post->post_title .'</h5>
                <p class="review-position"><i>'. get_post_meta($post->ID, 'bw-position', true) .'</i></p>
                '. the_review_socials( get_post_meta($post->ID, 'bw-socials', true), false ) .'
                <p>      
                    '. $post->post_content .'
                </p>
            </div>';
        ...
        $output .= '</div>
        <button class="next-arrow"><i class="fa fa-chevron-right"></i></button>
        <button class="prev-arrow"><i class="fa fa-chevron-left"></i></button>';
